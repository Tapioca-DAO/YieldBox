...
def append_semicolons_to_directives(line: str) -> Tuple[str, int]:
    if line.lstrip().startswith(('pragma', 'import', 'using')) and not line.rstrip().endswith(';'):
...
def main() -> int:
    ...
    for d in args.dirs:
        ...
        # TODO: If we want to make this script pulic for clients to use - remove the 'emv-' check.
        dir_files = filter(
            lambda fname: not any(p.startswith(s) for p in fname.parts for s in ('emv-', '.certora_config')),
    ...
    semicolon_in_directives_str = "'pragma', 'import' or 'using' directives appended with ';'"
